...
- `size` 仍然是三个方向上的半尺寸；
- `mass` 仍然是质量；
- `pos="0 0 -0.2"` 表示这个 geom 相对所属的 `link` body 再向 z 负方向偏移 0.2 m。
...
这里故意把重力设为 0，只是为了后面学习关节和 actuator 时，杆不会在还没施加控制之前自己掉下来。它和 falling box 中的 `option` 是同一个概念。
