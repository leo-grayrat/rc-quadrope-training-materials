...
原稿中用 `qpos[7:]` 说明自由基座后的关节位置。这种切片需要先确认模型结构。

自己整理机器狗时，可以根据 joint 名称查找对应位置：
...
`mj_name2id()` 把 XML 中的 joint 名称转换成 id，`jnt_qposadr` 给出该 joint 在 `qpos` 中的起始地址。

同样，调试 actuator 时也应先核对名称和目标 joint，而不是看到 `ctrl[0]` 就猜它控制哪条腿。

[starter/04_quadruped/model_info.py](./starter/04_quadruped/model_info.py) 会列出 joint 名称、`qpos` 地址以及 actuator 对应的 joint，可以用来检查自己转换后的模型。


### 关于 nu 与 actuator 数量

原稿把 `model.nu` 作为控制输入维度，并在当前四足任务中把它和执行器数量对应起来。对于本任务使用的普通 `motor` actuator，每个 actuator 只有一个控制输入，因此这个理解成立。

当前 MuJoCo 也支持具有多个控制输入的 actuator 类型，所以一般情况下 `model.nu` 与 `model.nactuator` 不一定相等。配套的 `model_info.py` 会同时打印这两个值；如果二者不同，就不能再把 actuator id 直接当作 `ctrl` 下标。
