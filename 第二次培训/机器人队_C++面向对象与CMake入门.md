# 机器人队 C++ 面向对象与 CMake 入门

这份讲义作为第二次培训的 C++ 前置，目标是建立后续机器人软件会反复使用的面向对象与基础工程能力。正式任务要求仍以同目录的 `c++python概念-任务.pdf` 为准；本文负责补足其中需要的 C++ 思维与工程基础。

内容从一个简单的 `Motor` 类开始，逐步引入封装、构造函数、抽象接口、继承与多态，再把电机组合进 `Robot`，最后把单文件程序拆成多文件工程并使用 CMake 构建。关键位置会给出任务、起始代码和可验证结果；第一次阅读时先完成 TODO，再展开参考实现。
